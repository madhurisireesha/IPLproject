...
public class IPLDataProject {
    ...
    private static void findPlayerWithHighestStrikeRatePerTeamByYear(List<Match> matches, List<Delivery> deliveries) {
        Map<String, Map<String, Map<String, Float>>> strikeRatePerPlayerInEveryTeamPerYear = new HashMap<>();
        Map<String, Map<String, Map<String, Float>>> highStrikeRatePerPlayerInEveryTeamPerYear = new HashMap<>();
        Map<String, List<String>> idsOfEveryYear = new HashMap<>();

        // Collect match IDs for each year
        for (Match match : matches) {
            String year = String.valueOf(match.getSeason());
            String matchId = String.valueOf(match.getId());
            idsOfEveryYear.putIfAbsent(year, new ArrayList<>());
            idsOfEveryYear.get(year).add(matchId);
        }

        // Process deliveries to calculate strike rates
        for (Delivery delivery : deliveries) {
            String matchId = String.valueOf(delivery.getMatchId());
            String year = getYearForMatch(idsOfEveryYear, matchId);

            if (year != null) {
                String team = delivery.getBattingTeam();
                String batsman = delivery.getBatsman();
                float runsPerBall = delivery.getBatsmanRuns();
                int over = delivery.getOver();

                // Process only overs 16 and onwards
                if (over >= 16) {
                    strikeRatePerPlayerInEveryTeamPerYear
                            .computeIfAbsent(year, k -> new HashMap<>())
                            .computeIfAbsent(team, k -> new HashMap<>())
                            .merge(batsman, runsPerBall, Float::sum);
                }
            }
        }

        // Calculate strike rates for players
        for (String year : strikeRatePerPlayerInEveryTeamPerYear.keySet()) {
            for (String team : strikeRatePerPlayerInEveryTeamPerYear.get(year).keySet()) {
                Map<String, Float> playerRuns = strikeRatePerPlayerInEveryTeamPerYear.get(year).get(team);
                for (String player : playerRuns.keySet()) {
                    float totalRuns = playerRuns.get(player);
                    // Assume balls faced are also tracked and available
                    float ballsFaced = getBallsFacedForPlayer(year, team, player); // Replace with actual data
                    float strikeRate = (ballsFaced > 0) ? (totalRuns * 100) / ballsFaced : 0.0f;
                    // Store the strike rate
                    strikeRatePerPlayerInEveryTeamPerYear.get(year).get(team).put(player, strikeRate);
                }
            }
        }

        // Find player with the highest strike rate per team by year
        for (String year : strikeRatePerPlayerInEveryTeamPerYear.keySet()) {
            for (String team : strikeRatePerPlayerInEveryTeamPerYear.get(year).keySet()) {
                Map<String, Float> playerStrikeRates = strikeRatePerPlayerInEveryTeamPerYear.get(year).get(team);
                String bestPlayer = null;
                float highestStrikeRate = 0.0f;

                for (Map.Entry<String, Float> entry : playerStrikeRates.entrySet()) {
                    if (entry.getValue() > highestStrikeRate) {
                        highestStrikeRate = entry.getValue();
                        bestPlayer = entry.getKey();
                    }
                }

                if (bestPlayer != null) {
                    highStrikeRatePerPlayerInEveryTeamPerYear
                            .computeIfAbsent(year, k -> new HashMap<>())
                            .computeIfAbsent(team, k -> new HashMap<>())
                            .put(bestPlayer, highestStrikeRate);
                }
            }
        }

        // Print the results
        for (String year : highStrikeRatePerPlayerInEveryTeamPerYear.keySet()) {
            System.out.println("Year: " + year);
            for (String team : highStrikeRatePerPlayerInEveryTeamPerYear.get(year).keySet()) {
                System.out.println("Team: " + team + ", Player with highest strike rate: " +
                        highStrikeRatePerPlayerInEveryTeamPerYear.get(year).get(team));
            }
        }
    }
     private static String getYearForMatch(Map<String, List<String>> idsOfEveryYear, String matchId) {
        for (Map.Entry<String, List<String>> entry : idsOfEveryYear.entrySet()) {
    ...
    private static float getBallsFacedForPlayer(String year, String team, String player) {
